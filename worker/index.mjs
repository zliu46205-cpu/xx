    ...
    const response = await fetch("https://api.deepseek.com/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: `请基于以下输入生成${tier.name}。档位要求：${tier.guidance}。术数专项要求：${input.methodGuide}。问题场景：${input.questionScene}。请明显区分本方法和其他方法，避免套话。只返回 JSON，不要 Markdown。不要照抄 baseReportDigest 的原文；必须根据用户原问、method.id、questionScene 和 reportStructure 重写 summary、situation、tendency、inference、suggestions；报告章节和推演顺序必须服从 reportStructure.sections。${retryHint}\n${JSON.stringify(input)}` },
        ],
